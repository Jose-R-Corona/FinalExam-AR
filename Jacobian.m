function Jq1 = Jacobian(q,L)
    ...
    H = (Rz(q(1))*Tz(L(1))*Rx(q(2))*Tz(L(2))*Rx(-q(3))*Tz(L(3))*Rx(q(4))*Rz(q(5))*Tz(L(4))*Rx(q(6)));% forward kinematics
    %H=simplify(H);
    R = H(1:3,1:3);
    %R = simplify(H(1:3,1:3));  % extract rotation matrix
    % diff by q1
    Td=(Rzd(q(1))*Tz(L(1))*Rx(q(2))*Tz(L(2))*Rx(-q(3))*Tz(L(3))*Rx(q(4))*Rz(q(5))*Tz(L(4))*Rx(q(6)))*...
